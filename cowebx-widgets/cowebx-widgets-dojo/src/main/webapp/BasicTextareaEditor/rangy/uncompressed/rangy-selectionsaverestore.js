...
rangy.createModule("SaveRestore", function(api, module) {
    ...
    function insertRangeBoundaryMarker(range, atStart, start) {
        if(start)
            var markerId = "selectionBoundary_" + '1';
        else
            var markerId = "selectionBoundary_" + '2';
        var markerEl;
    ...
    function saveSelection(win) {
        ...
        for (var i = 0, len = ranges.length; i < len; ++i) {
            range = ranges[i];
            if (range.collapsed) {
                endEl = insertRangeBoundaryMarker(range, false, false);
                rangeInfos.push({
                    markerId: endEl.id,
                    collapsed: true
                });
                ...
                endEl = insertRangeBoundaryMarker(range, false, false);
                startEl = insertRangeBoundaryMarker(range, true, true);
