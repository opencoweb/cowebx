...
(function(
			...
			config = function(config, booting){
				...
				for(p in config){
					...
					if(config[p]!==hasCache){
						...
						has.add(undefined+p, config[p], 0, booting);
					}
				}
	...
	var def = function(
		...
		}else if(has("dom-addeventlistener") || !1){
...
define([
	...
	"./_base/browser"], function(dojo, has, require){
	// module:
	//		./_base/browsermain
	// summary:
	...
	// loading dojo.js or explicitly include ./_base/browser_firebug/firebug in a dependency list.
...
define(["./kernel", "../has"], function(dojo, has){
	...
	has.add(undefined, 1);
	has.add(undefined, dojo.isOpera= isOpera);
	has.add(undefined, dojo.isAIR= isAIR);
	has.add(undefined, dojo.isKhtml= isKhtml);
	has.add(undefined, dojo.isWebKit= isWebKit);
	has.add(undefined, dojo.isChrome= isChrome);
	has.add(undefined, dojo.isMac = isMac );
	has.add(undefined, dojo.isSafari= isSafari);
	has.add(undefined, dojo.isMozilla = dojo.isMoz = isMozilla );
	has.add("ie", dojo.isIE = isIE );
	has.add(undefined, dojo.isFF= isFF);
	has.add(undefined, dojo.isQuirks= isQuirks);
...
define(["require"], function(require) {
	...
	if(!has("dojo-has-api")){
		...
		has.add = function(name, test, now, force){
			...
			//	|				has.add(undefined, function(){ return true; }, true);
			//
			// example:
			//			Again with the redundantness. You can do this in your tests, but we should
			//			not be doing this in any internal has.js tests
			//	|				has.add(undefined, true);
			//
			// example:
			//			Three things are passed to the testFunction. `global`, `document`, and a generic element
			//			from which to work your test should the need arise.
			//	|				has.add(undefined, function(g, d, el){
	...
	if(1){
		...
		has.add(undefined, screen.availWidth || innerWidth);
		has.add(undefined, !!agent.match(/iPhone|iP[ao]d/));
		has.add(undefined, agent.indexOf("android") > 1);
	}
